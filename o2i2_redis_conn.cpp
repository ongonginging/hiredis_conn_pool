...
REDIS_RESULT init_redis_pool(RedisConnCBPool** pool, 
	...
	*pool = construct_pool(size, host, port, timeout, retry_times, logger);
	if (NULL == pool){
		logger(ERROR, "[hiredis_conn_pool|%s|%d|%s] Do construct_pool failed.", __file__, __line__, __func__);
...
REDIS_RESULT deinit_redis_pool(RedisConnCBPool* pool){
	...
	if (false == destruct_rv){
		logger(ERROR, "[hiredis_conn_pool|%s|%d|%s] Do destruct_pool failed.", __file__, __line__, __func__);
...
REDIS_RESULT do_redis_command(RedisConnCBPool* pool, redisReply** reply, char* cmd, ...){
	...
	if (NULL == cb->context){
		...
		if(false == conn_rv){
			pool->logger(ERROR, "[hiredis_conn_pool|%s|%d|%s] Do conn failed.", __file__, __line__, __func__);
			rv = REDIS_RESULT_CONNECT_SERVER_FAILED;
			bool push_rv = false;
			push_rv = push_cb(pool, cb);
			if (false == push_rv) {
				pool->logger(ERROR, "[hiredis_conn_pool|%s|%d|%s] Do push_cb failed.", __file__, __line__, __func__);
